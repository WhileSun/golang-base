...
import (
	...
	"github.com/whilesun/go-admin/app/types/vo"
...
func (s *SysMenuService) checkPagePermsExist(menuModel *models.SMenu) error {
...
func (s *SysMenuService) checkTypeParams(menu *models.SMenu) error {
	if menu.MenuType == 1 {
		if menu.Url == "" {
...
func (s *SysMenuService) addPagePermsMenu(tx *gorm.DB, keys []string, parentId int, dataPermsHeader string) error {
	...
	if err := tx.Create(menus).Error; err != nil {
		gctx.Logger.Error("添加菜单栏目子节点失败—>", err.Error())
		return errors.New("添加菜单栏目子节点失败！")
	}
	return nil
}

func (s *SysMenuService) Add(params dto.AddMenu) error {
	err := gctx.Db.Transaction(func(tx *gorm.DB) error {
		...
		if err := SysMenuServiceApp.checkTypeParams(menuModel); err != nil {
			return err
		}
		if menuModel.MenuType == 2 {
			if err := SysMenuServiceApp.checkPagePermsExist(menuModel); err != nil {
				return err
			}
		}
		if err := tx.Create(menuModel).Error; err != nil {
			gctx.Logger.Error("添加菜单栏目失败—>", err.Error())
			return errors.New("添加菜单栏目失败！")
		}
		//只有菜单栏有节点
		if menuModel.MenuType == 1 && len(params.TargetKeys) > 0 {
			if err := SysMenuServiceApp.addPagePermsMenu(tx, params.TargetKeys, menuModel.Id, params.DataPermsHeader); err != nil {
				return err
			}
		}
		return nil
...
func (s *SysMenuService) Update(params dto.UpdateMenu) error {
	err := gctx.Db.Transaction(func(tx *gorm.DB) error {
		//操作权限标识转大写
		params.PagePerms = strings.ToUpper(params.PagePerms)
		...
		if menuModel.MenuType == 1 {
			...
			if len(addKeys) > 0 {
				if err := SysMenuServiceApp.addPagePermsMenu(tx, addKeys, menuModel.Id, params.DataPermsHeader); err != nil {
					return err
				}
			}
		...
		if err := menuModel.Update(tx); err != nil {
			gctx.Logger.Error("编辑菜单栏目失败—>", err.Error())
...
func (s *SysMenuService) Delete(params dto.DeleteMenu) error {
	oldMenuModel := models.NewMenu()
	oldMenuModel.GetRow(params.Id)
	if oldMenuModel.Id == 0 {
	...
	if err != nil {
		gctx.Logger.Error("删除菜单栏目失败—>", err.Error())
...
func (s *SysMenuService) GetUserRouteList(roleIdents []string) []*MenuRoutesResult {
	menuModel := models.NewMenu()
